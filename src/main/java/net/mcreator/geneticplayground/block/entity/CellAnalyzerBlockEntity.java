...
public class CellAnalyzerBlockEntity extends RandomizableContainerBlockEntity implements WorldlyContainer {
	private NonNullList<ItemStack> stacks = NonNullList.withSize(5, ItemStack.EMPTY);

	public CellAnalyzerBlockEntity(BlockPos position, BlockState state) {
		super(GeneticPlaygroundModBlockEntities.CELL_ANALYZER.get(), position, state);
	}
	...
	public Component getDefaultName() {
		return Component.literal("cell_analyzer");
	}

	@Override
	public AbstractContainerMenu createMenu(int id, Inventory inventory) {
		return new CellAnalyzerGUIMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(this.worldPosition));
	}
