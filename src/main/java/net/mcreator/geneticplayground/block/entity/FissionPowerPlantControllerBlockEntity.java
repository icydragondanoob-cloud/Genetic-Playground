...
public class FissionPowerPlantControllerBlockEntity extends RandomizableContainerBlockEntity implements WorldlyContainer {
	private NonNullList<ItemStack> stacks = NonNullList.withSize(0, ItemStack.EMPTY);

	public FissionPowerPlantControllerBlockEntity(BlockPos position, BlockState state) {
		super(GeneticPlaygroundModBlockEntities.FISSION_POWER_PLANT_CONTROLLER.get(), position, state);
	}
	...
	public Component getDefaultName() {
		return Component.literal("fission_power_plant_controller");
	}

	@Override
	public AbstractContainerMenu createMenu(int id, Inventory inventory) {
		return new FissionPowerPlantGUIMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(this.worldPosition));
	}
	...
	private final FluidTank fluidTank0 = new FluidTank(100000, fs -> {
	...
	private final FluidTank fluidTank1 = new FluidTank(110000, fs -> {
	...
	private final FluidTank fluidTank2 = new FluidTank(85000, fs -> {
		if (fs.getFluid() == GeneticPlaygroundModFluids.STERILE_BLOOD.get())
			return true;
		return false;
	}) {
		@Override
	...
	private final FluidTank[] fluidTanks = {fluidTank0, fluidTank1, fluidTank2};
	private final FluidTank[] ioTanks = {fluidTank0};
	private final FluidTank[] inputTanks = {fluidTank1, fluidTank2};
