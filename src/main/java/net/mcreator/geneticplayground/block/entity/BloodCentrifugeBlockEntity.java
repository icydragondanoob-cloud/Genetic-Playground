...
public class BloodCentrifugeBlockEntity extends RandomizableContainerBlockEntity implements WorldlyContainer {
	private NonNullList<ItemStack> stacks = NonNullList.withSize(5, ItemStack.EMPTY);

	public BloodCentrifugeBlockEntity(BlockPos position, BlockState state) {
		super(GeneticPlaygroundModBlockEntities.BLOOD_CENTRIFUGE.get(), position, state);
	}
	...
	public Component getDefaultName() {
		return Component.literal("blood_centrifuge");
	}

	@Override
	public AbstractContainerMenu createMenu(int id, Inventory inventory) {
		return new BloodCentrifugeGUIMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(this.worldPosition));
	}
	...
	private final EnergyStorage energyStorage = new EnergyStorage(50000, 50000, 50000, 0) {
		@Override
	...
	private final FluidTank fluidTank0 = new FluidTank(3150, fs -> {
